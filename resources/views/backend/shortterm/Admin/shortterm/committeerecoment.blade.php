 ...
 <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Recommend By Committee Application
    ...
    <div class="card mb-3">
		<div class="card-header text-center"><h4 class="mt-2">Recommend By Committee Application
</h4></div>
        ...
        <!-- <a class="btn btn-success" href="{{ url('short-term-application-consider/'.$record[0]->short_term_id) }}"><i class="nav-icon fas "></i>Consider</a>
         ...
         $roleid = \App\User::with('roles')->find($officer_id);
          ...
          ?> 
          @if($roleid->role!=3)
         <button type="button" class="btn btn-primary" data-toggle="modal" style="border: #3c8424;background-color: #3c8424;" onclick="considered_nonconidered(3,'<?php echo $record[0]->short_term_id;?>' )">Consider</button>
		...
		@endif
		<a href="{{url('recommend-by-committe-short-term')}}" class="btn btn-secondary">Back</a>
...
function consider_form_sumbit(){
 ...
 var page_url = "{{ url('forward-committee') }}"+"/"+student_id;
  ...
  if(remarks == ""){
    ...
    $.ajax({
            url:"{{ url('short-term-application-selection') }}",
            type: 'POST',
